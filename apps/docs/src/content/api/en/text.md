---
title: Text API
routeSlug: text
locale: en
docs: /en/components/text/spec
designOrder: 150
developOrder: 80
---

<!-- AUTO-GENERATED:START -->

## Import

```ts
import { RText } from "@ripple-design/rui"
```

## Props

| Name | Type | Default | Required | Description |
| --- | --- | --- | --- | --- |
| `variant` | `RTextVariant` | `"body1"` | no | Controls the typography style applied to the content. |
| `as` | `string` | — | no | Overrides the semantic HTML tag chosen from the active variant. |
| `emphasis` | `RTextEmphasis` | `"high"` | no | Controls the foreground emphasis level. |
| `color` | `RTextColor` | — | no | Chooses a semantic foreground color instead of the inherited surface color. |
| `disabled` | `boolean` | `false` | no | Forces low emphasis and applies the disabled ARIA state. |
| `ellipsize` | `"end"` | — | no | Truncates overflowing single-line text at the end with an ellipsis. |

Generated from `packages/rui/src/components/text/types.ts`.

<!-- AUTO-GENERATED:END -->
