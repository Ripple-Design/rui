...
import type { RDataTableModelState, RDataTableServerEmits, RDataTableServerProps, RDataTableSlots } from "./types"

const props = defineProps<RDataTableServerProps<T>>()
const emit = defineEmits<RDataTableServerEmits>()
...
<template>
    <RDataTableCore v-bind="props" :model-state="modelState" server @update:page="emit('update:page', $event)" @update:items-per-page="emit('update:itemsPerPage', $event)" @update:sort-by="emit('update:sortBy', $event)" @update:group-by="emit('update:groupBy', $event)" @update:options="emit('update:options', $event)">
