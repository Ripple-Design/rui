...
import { navigationDrawerGroupKey, navigationDrawerKey } from "./context"
import type { RNavigationDrawerItemProps } from "./types"
...
const rippleOptions = computed<RippleOptions>(() => {
    ...
    if (props.ripple === true || props.ripple == null) return { disabled: false, contrast: "low", getSurfaceTarget: () => indicatorRef.value }
    return {
        ...props.ripple,
...
const classes = computed(() => [
    ...
    { "rui-navigation-drawer-item--selected": selected.value, "rui-navigation-drawer-item--icon-aligned": group?.hasIcon.value },
])

watch(
    [() => props.value, () => attrs.disabled, interactiveRef, () => drawer],
    ([value, disabled, element, nextDrawer]) => nextDrawer?.registerItem(itemId, { value, element, disabled: !!disabled }),
    { immediate: true },
)
