...
import type { RNumberPickerProps } from "./types.ts"
...
const displayedValue = computed(() =>
    valueAtIndex(resolveIndex(trackCenterIndex.value, range.value, props.wrap), range.value),
)
...
let pointer: { id: number; lastTime: number; lastY: number; moved: boolean; startY: number; velocity: number } | null =
    null
let frame = 0
let animation: {
    elapsed: number
...
function canScroll(delta: number) {
    ...
    return (
        !(delta > 0 && trackCenterIndex.value <= 0) && !(delta < 0 && trackCenterIndex.value >= range.value.count - 1)
    )
}
...
function stepFling(time: number) {
    ...
    if (
        (!props.wrap && before === trackCenterIndex.value && trackOffset.value === 0) ||
        Math.abs(flingVelocity) < MINIMUM_FLING_VELOCITY
    ) {
        flinging = false
        animation = null
        ensureWheelAdjusted()
...
<template>
    ...
    >
        <span class="rui-number-picker__viewport" aria-hidden="true">
            <span
...
.rui-number-picker__item {
    position: absolute;
    inset-inline: 0;
    inset-block-start: calc(var(--rui-comp-number-picker-element-height) + var(--rui-comp-number-picker-item-gap) / 2);
    display: grid;
    place-items: center;
    ...
    transform: translateY(
        calc(
            var(--rui-comp-number-picker-item-offset) * var(--rui-comp-number-picker-element-height) +
                var(--rui-comp-number-picker-wheel-offset)
        )
    );
...
.rui-number-picker__selection {
    ...
    inset-block-start: calc(
        var(--rui-comp-number-picker-element-height) + var(--rui-comp-number-picker-item-gap) /
            2 - var(--rui-comp-number-picker-divider-thickness)
    );
