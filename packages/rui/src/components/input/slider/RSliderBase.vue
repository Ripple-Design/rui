...
import RSliderValueIndicator from "@/components/input/slider/RSliderValueIndicator.vue"
import { useFormField } from "@/components/layout/form/useFormField.ts"

import type { RSliderRangeValue, RSliderProps, RRangeSliderProps } from "./types.ts"

import RThumb from "../../internal/thumb/RThumb.vue"
import {
    formatSliderValue,
    percentToValue,
...
type RSliderModelValue = number | RSliderRangeValue
type SliderBaseProps = RSliderProps & Pick<RRangeSliderProps, "startAriaLabel" | "endAriaLabel"> & {
    mode: "single" | "range"
}

defineOptions({ inheritAttrs: false })

const props = withDefaults(defineProps<SliderBaseProps>(), {
    disabled: false,
    max: 100,
    min: 0,
    showTicks: false,
