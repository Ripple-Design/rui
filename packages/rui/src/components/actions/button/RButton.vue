...
import type { RButtonProps, RButtonType, RButtonVariant } from "./types.ts"

import { buttonGroupKey, buttonRowVariantKey } from "./groupContext.ts"
import {
...
const resolvedVariant = computed<RButtonVariant>(
    () => props.variant ?? group?.variant.value ?? rowVariant?.value ?? "contained",
)
const iconSize = computed(() => (isIconGroup.value ? 24 : 18))
const rippleOptions = computed(() =>
    resolveButtonRippleOptions(resolvedVariant.value, props.ripple, resolvedDisabled.value),
)
