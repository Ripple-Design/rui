...
import type { RChipGroupModelValue, RChipGroupProps } from "./types.ts"

import { chipGroupKey, type RChipGroupItemState } from "./groupContext.ts"
...
function activate(id: symbol) {
    ...
    if (props.selection === "multiple") {
        if (isSelected(value)) {
            ...
            formField.setValue(
                multipleValue.value.filter((entry) => !Object.is(entry, value)),
                "change",
            )
            return
        }
