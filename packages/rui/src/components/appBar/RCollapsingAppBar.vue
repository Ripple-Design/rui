...
const titleAppearanceStyle = computed(() => ({
    ...
    "--rui-comp-app-bar-expanded-title-color": props.expandedTitleAppearance.color ?? "var(--rui-comp-surface-content-color)",
...
function updateTitleMetrics() {
    ...
    const expandedTextX = expandedStyle.textAlign === "center" ? (expandedRect.width - expandedElement.clientWidth) / 2 : 0
    const expandedTextY = (expandedRect.height - Number.parseFloat(expandedStyle.lineHeight)) / 2
    const collapsedTextX = collapsedStyle.textAlign === "center" ? (collapsedRect.width - collapsedElement.clientWidth) / 2 : 0
