...
import type { RNavigationDrawerSide } from "./types.ts"
...
function settleDuration(distance: number, width: number, velocity: number) {
    if (Math.abs(velocity) > 0)
        return Math.min(SETTLE_MAX_DURATION, 4 * Math.round((1000 * distance) / Math.abs(velocity)))
...
export function useNavigationDrawerDrag(options: UseNavigationDrawerDragOptions) {
    ...
    function finish(target: 0 | 1) {
        ...
        const duration = matchMedia("(prefers-reduced-motion: reduce)").matches
            ? 0
            : settleDuration(distance, width, velocity)
    ...
    function handlePointerEnd(event: PointerEvent) {
        ...
        dragging.value = false
        const target =
            velocity > VELOCITY_THRESHOLD ? 1 : velocity < -VELOCITY_THRESHOLD ? 0 : progress.value >= 0.5 ? 1 : 0
        finish(target)
    }
