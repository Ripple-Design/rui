...
import type { RScaffoldScrollDirection, RScaffoldScrollFacts } from "@/components/layout/scaffold/context.ts"

import type { RAppBarScrollState } from "./context.ts"
import type { RAppBarScrollBehavior } from "./types.ts"
...
export function useAppBarScroll(options: AppBarScrollOptions) {
    ...
    function publish() {
        ...
        const hidden = Boolean(
            toValue(options.hideOnScroll) && facts.direction === "down" && facts.top > collapseDistance,
        )
        const phase = hidden
            ? "hidden"
            : collapseProgress >= 1
