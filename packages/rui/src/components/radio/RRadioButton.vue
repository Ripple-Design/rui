...
import { radioButtonGroupKey } from "./groupContext"
import type { RRadioButtonProps } from "./types"
...
function handleKeydown(event: KeyboardEvent) {
    ...
    const before = group.orientation.value === "vertical" ? ["ArrowUp", "ArrowDown", "Home", "End"] : ["ArrowLeft", "ArrowRight", "Home", "End"]
...
@keyframes rui-radio-button-dot-on {
    0%, 33.333% { transform: scale(0); }
    36.546% { transform: scale(1.5); }
    100% { transform: scale(1); }
}
...
@keyframes rui-radio-button-dot-off {
    0% { transform: scale(1); }
    36.673% { transform: scale(1.4); }
    39.880%, 100% { transform: scale(0); }
}
