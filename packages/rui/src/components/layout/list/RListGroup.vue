...
import type { RListGroupProps } from "./types.ts"
...
<template>
    <li v-bind="attrs" :class="['rui-list-group', { 'rui-list-group--divider': divider }]">
        <div v-if="hasHeading || $slots.header" :id="headingId" class="rui-list-group__header">
            <slot name="header">{{ label }}</slot>
        </div>
        <ul class="rui-list-group__items" :aria-labelledby="hasHeading || $slots.header ? headingId : undefined">
            <slot />
        </ul>
    </li>
</template>
