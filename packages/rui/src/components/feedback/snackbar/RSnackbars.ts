...
import type { RSnackbarDismissReason, RSnackbarHandle, RSnackbarOptions } from "./types.ts"
...
function show(options: string | RSnackbarOptions): RSnackbarHandle {
    ...
    return {
        ...
        update: (updates) => {
            const target =
                state.current?.id === record.id ? state.current : state.next?.id === record.id ? state.next : null
            if (target) {
